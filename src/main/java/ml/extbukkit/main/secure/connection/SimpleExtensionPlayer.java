...
import ml.extbukkit.main.secure.world.entity.ExtensionedEntity;
...
public class SimpleExtensionPlayer extends ExtensionedEntity implements ExtensionedPlayer {
  private Player base;
  ...
  @Override
  public void sendActionbar(String message, Extension extension, int stayTime) {
    sendActionbar(new ChatMessage(message), extension, stayTime);
  }

  @Override
  public void sendActionbar(ChatMessage message, Extension extension, int stayTime) {
    new Runnable() {
      ScheduledTask task = server.getSchedulerManager().schedule(extension, this, new Time(0), new Time(20));
      int stayed;
